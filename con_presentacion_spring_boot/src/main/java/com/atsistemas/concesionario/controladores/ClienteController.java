...
public class ClienteController {
    ...
    public String alta(@ModelAttribute @Valid Cliente cliente, HttpSession session) {
        ...
        restTemplate.postForObject("https://localhost:8080/con_rest/api/cliente/alta", cliente, Cliente.class, headers);
    ...
    public String lista(Model modelo, HttpSession session){
        SecurityTools.setAuthority(restTemplate, (String)session.getAttribute("login"));
        List<Cliente> lista = restTemplate.getForObject("https://localhost:8080/con_rest/api/cliente/lista", List.class);
        ...
        //modelo.addAttribute("comercial", restTemplate.getForObject("https://localhost:8080/con_rest/api/comercial/2", Comercial.class));
        modelo.addAttribute("comerciales", restTemplate.getForObject("https://localhost:8080/con_rest/api/comercial/lista", List.class));
    ...
    public String baja(@ModelAttribute @Valid Cliente cliente, HttpSession session){
        ...
        SecurityTools.setAuthority(restTemplate, (String)session.getAttribute("login"));
        SecurityTools.setContentTypeJSON(headers);
        restTemplate.put("https://localhost:8080/con_rest/api/cliente/baja", cliente, headers);
        return "redirect:lista";
    }
    ...
    public String detalle(@PathVariable int id, Model modelo, HttpSession session){
        SecurityTools.setAuthority(restTemplate, (String)session.getAttribute("login"));
        Cliente c = restTemplate.getForObject("https://localhost:8080/con_rest/api/cliente/"+id, Cliente.class);
    ...
    public String modifica(@ModelAttribute @Valid Cliente cliente, HttpSession session){
        ...
        SecurityTools.setAuthority(restTemplate, (String)session.getAttribute("login"));
        SecurityTools.setContentTypeJSON(headers);
        restTemplate.postForObject("https://localhost:8080/con_rest/api/cliente/actualizar", cliente, Cliente.class, headers);
        return "redirect:lista";
    }
